            ...
            {sorteddata.map((e: any, i: number) => (
              <Link href={`/${e.slug}`} key={e._id}>
                <AnimatePresence>
                    <AnimatedWords2
                      title={(i + 1 + '.') + ' ' + e.title}
                    style={`flex p-5 hover:text-white hover:border-red-500 hover:pl-16 duration-150 hover:bg-red-500 border-t border-red-500  flex-wrap w-[98%] items-start text-left text-2xl lg:text-4xl font-neue  `}
                    />
